...
// Adapted for npm @supabase/supabase-js (no CDN)
...
export async function unfreezeGame(gameId, joinerId) {
  ...
  if (!game || game.status !== 'frozen')
    throw new Error('Game is not frozen.');
  if (game.leaver_id !== joinerId)
    throw new Error('Only the player who left can rejoin.');
  if (Date.now() - new Date(game.leave_time).getTime() > 10 * 60 * 1000) {
    await terminateGame(game.id);
...
export async function syncOfflineToCloud(userId) {
  ...
  const aiBackup = JSON.parse(
    localStorage.getItem('chess3d_backup_ai') || 'null'
  );
  const pvpBackup = JSON.parse(
    localStorage.getItem('chess3d_backup_2p') || 'null'
  );
