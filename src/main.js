...
async function init() {
    ...
    try {
        ...
        engine.setFrameCallback((state) => {
            ...
            if (state.inCheck) document.getElementById('smsg').textContent = '⚠ Check!';
            else if (!state.aiThink) document.getElementById('smsg').textContent = '';
            if (state.over) {
...
async function updateHeaderWithAvatar() {
    if (currentUserId && db) {
        const p = await db.fetchProfileData(currentUserId);
        ui.updateHeaderUI(currentUserId, p.avatar_url);
...
function enterOnlineGame(game) {
    ...
    engine.syncBoardFromServer(
        game.board_state.brd,
        game.board_state.turn,
        game.board_state.cas,
        game.board_state.ep,
        game.timer_w,
        game.timer_b
    );
...
async function syncTimers() {
    ...
    try {
        await db.sb
            .from('online_games')
            .update({ timer_w: engine.getTimerW(), timer_b: engine.getTimerB() })
...
async function createPublicRoom() {
    await createRoom(null, 'public');
}

async function createPrivateRoom() {
    await createRoom(null, 'private');
}
...
async function createRoom(code, type) {
    ...
    if (!username) {
        ui.toast('Please set a username.');
        return;
    }
...
function startChatPolling(gameId) {
    ...
    chatPollInterval = setInterval(async () => {
        try {
            ui.displayChatMessages(await db.getChatMessages(gameId));
        } catch (e) {}
    }, 2000);
}
...
async function sendChat(msg) {
    if (!currentOnlineGame) return;
    const nickname =
        currentOnlineGame.host_player_id === currentUserId
...
async function requestRematch() {
    ...
    await db.sb
        .from('online_games')
        .update({ rematch_requested_by: currentUserId, rematch_requested_at: new Date() })
        .eq('id', currentOnlineGame.id);
    document.getElementById('gos').textContent = 'Rematch requested...';
}
...
function saveBackup() {
    ...
    localStorage.setItem('chess3d_backup_' + gameMode, JSON.stringify(engine.getBackupData()));
}
...
function restoreLocalGame() {
    const ai = localStorage.getItem('chess3d_backup_ai'),
        pvp = localStorage.getItem('chess3d_backup_2p');
    if (!ai && !pvp) {
...
async function syncOfflineToCloud() {
    if (!currentUserId) {
        ui.toast('Please log in to sync data.', 3000);
        return;
    }
    if (!navigator.onLine) {
        ui.toast('No internet connection.', 3000);
        return;
    }
    ...
    if (!aiBackup && !pvpBackup) {
        ui.toast('No offline data to sync.', 4000);
        return;
    }
...
async function restoreOfflineFromCloud() {
    if (!currentUserId) {
        ui.toast('Please log in to restore cloud data.', 3000);
        return;
    }
    if (!navigator.onLine) {
        ui.toast('No internet connection.', 3000);
    ...
    try {
        ...
        ui.toast('Restore failed: ' + e.message, 4000);
        showError('restore', e);
    }
}
...
async function deleteAllSyncedData() {
    if (!currentUserId) {
        ui.toast('Please log in to delete synced data.', 3000);
        return;
    }
    if (!navigator.onLine) {
        ui.toast('No internet connection.', 3000);
    ...
    try {
        ...
        ui.toast('Cloud data deleted.', 3000);
    } catch (e) {
        ui.toast('Delete failed: ' + e.message, 4000);
        showError('delete', e);
    }
}
